...
// Upcoming = confirmed or pending; shows Reschedule + Cancel
// Past/Cancelled = completed or cancelled; shows Book Again, red/gray accents
const isUpcoming = (status: AppointmentStatus) => status === "confirmed" || status === "pending";

export default function AppointmentCard({
  dayNum, monthYear, dayLabel, service, location, stylist, time, status, price, onBookAgain,
}: Props) {
  ...
  const borderLeft =
    ...
    :                        "border-l-gray-200";   // completed — blends with outer border
  ...
  return (
    <div className={`border border-gray-200 border-l-4 ${borderLeft} rounded-[10px] p-5 flex items-center gap-6 mb-4`}>
      ...
      <div className="shrink-0 flex flex-col items-end gap-2">
        ...
        {upcoming ? (
          <>
            <button className="w-28 py-1.5 text-sm border border-gray-300 text-black rounded-[8px] hover:bg-gray-50 transition-colors">
              Reschedule
            </button>
            <button className="w-28 py-1.5 text-sm border border-red-400 text-red-500 rounded-[8px] hover:bg-red-50 transition-colors">
              Cancel
            </button>
          </>
        ) : (
